	...
	public ResponseEntity<List<EquipmentResponseDto>> getAllEquipments(){
	 ...
	 @GetMapping( "/{id}" )
		...
		public ResponseEntity<String> updateEquipment( @RequestBody EquipmentRequestDto equipmentRequestDto ) {
			...
			if ( isUpdated )
				return ResponseEntity.status( HttpStatus.CREATED ).body( MessageConstants.UPDATE__EQIPMENT_SUCCESS_MESSAGE );

			else
				return ResponseEntity.status( HttpStatus.BAD_REQUEST ).body( MessageConstants.UPDATE__EQIPMENT_ERROR_MSG );
		}
}
